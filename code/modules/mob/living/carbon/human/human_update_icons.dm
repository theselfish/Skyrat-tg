...
/mob/living/carbon/human/update_inv_gloves()
	...
	//Bloody hands begin
	var/mutable_appearance/bloody_overlay = mutable_appearance('modular_skyrat/modules/better_blood/icons/blood.dmi', "bloodyhands", -GLOVES_LAYER) // SKYRAT EDIT CHANGE
	cut_overlay(bloody_overlay)
	if(!gloves && blood_in_hands && (num_hands > 0))
		bloody_overlay = mutable_appearance('modular_skyrat/modules/better_blood/icons/blood.dmi', "bloodyhands", -GLOVES_LAYER) // SKYRAT EDIT CHANGE
		if(num_hands < 2)
			if(has_left_hand(FALSE))
				bloody_overlay.icon_state = "bloodyhands_left"
			else if(has_right_hand(FALSE))
				bloody_overlay.icon_state = "bloodyhands_right"
		bloody_overlay.color = get_blood_dna_color(return_blood_DNA()) // SKYRAT EDIT ADDITION
